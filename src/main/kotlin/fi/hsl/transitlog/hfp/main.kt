...
fun main(vararg args: String) {
    ...
    try {
        ...
        val unhealthyIfNoMessage =
            app.context.config!!.getDuration("application.unhealthyIfNoMessage")
        // Service is considered unhealthy if no message is acknowledged within this duration
        // Use long enough duration for this value, files contain data for one hour and are uploaded
        // after they have not been modified for one hour
        val unhealthyIfNoAck = app.context.config!!.getDuration("application.unhealthyIfNoAck")

        app.context.healthServer?.addCheck {
            val timeSinceLastHandled =
                Duration.ofNanos(System.nanoTime() - messageHandler.getLastHandledMessageTime())
            // There should be new messages coming in every second
            val healthy = timeSinceLastHandled < unhealthyIfNoMessage

            if (!healthy) {
                log.warn {
                    "Service unhealthy, last message handled ${timeSinceLastHandled.toMinutes()} minutes ago"
                }
            }
        ...
        app.context.healthServer?.addCheck {
            val timeSinceLastAcknowledged =
                Duration.ofNanos(
                    System.nanoTime() - messageHandler.getLastAcknowledgedMessageTime()
                )
            ...
            if (!healthy) {
                log.warn {
                    "Service unhealthy, last message acknowledged ${timeSinceLastAcknowledged.toMinutes()} minutes ago"
                }
            }
