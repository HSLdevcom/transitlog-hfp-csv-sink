...
data class LightPriorityEvent(
    ...
    val tlpRequestId: Int? = null,
    val tlpRequestType: String? = null,
    val tlpPriorityLevel: String? = null,
    val tlpReason: String? = null,
    val tlpAttSeq: Int? = null,
    val tlpDecision: String? = null,
    val sid: Int? = null,
    val signalGroupId: Int? = null,
    val tlpSignalGroupNbr: Int? = null,
    val tlpLineConfigId: Int? = null,
    val tlpPointConfigId: Int? = null,
    val tlpFrequency: Int? = null,
    val tlpProtocol: String? = null
) : IEvent() {
    companion object {
        fun parse(hfpTopic: Hfp.Topic, hfpPayload: Hfp.Payload): LightPriorityEvent {
            return LightPriorityEvent(
                ...
                if (hfpPayload.hasTlpRequestid()) {
                    hfpPayload.tlpRequestid
                } else {
                    null
                },
                if (hfpPayload.hasTlpRequesttype()) {
                    hfpPayload.tlpRequesttype.toString()
                } else {
                    null
                },
                if (hfpPayload.hasTlpPrioritylevel()) {
                    hfpPayload.tlpPrioritylevel.toString()
                } else {
                    null
                },
                if (hfpPayload.hasTlpReason()) {
                    hfpPayload.tlpReason.toString()
                } else {
                    null
                },
                if (hfpPayload.hasTlpAttSeq()) {
                    hfpPayload.tlpAttSeq
                } else {
                    null
                },
                if (hfpPayload.hasTlpDecision()) {
                    hfpPayload.tlpDecision.toString()
                } else {
                    null
                },
                if (hfpPayload.hasSid()) {
                    hfpPayload.sid
                } else {
                    null
                },
                if (hfpPayload.hasSignalGroupid()) {
                    hfpPayload.signalGroupid
                } else {
                    null
                },
                if (hfpPayload.hasTlpSignalgroupnbr()) {
                    hfpPayload.tlpSignalgroupnbr
                } else {
                    null
                },
                if (hfpPayload.hasTlpLineConfigid()) {
                    hfpPayload.tlpLineConfigid
                } else {
                    null
                },
                if (hfpPayload.hasTlpPointConfigid()) {
                    hfpPayload.tlpPointConfigid
                } else {
                    null
                },
                if (hfpPayload.hasTlpFrequency()) {
                    hfpPayload.tlpFrequency
                } else {
                    null
                },
                if (hfpPayload.hasTlpProtocol()) {
                    hfpPayload.tlpProtocol
                } else {
                    null
                }
            )
