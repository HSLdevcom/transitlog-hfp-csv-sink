...
class MessageHandler(private val pulsarApplicationContext: PulsarApplicationContext) :
    ...
    private val dataDirectory: Path = Paths.get("hfp").also { Files.createDirectories(it) }

    private val validators: List<EventValidator> =
        mutableListOf<EventValidator>().also {
            if (config.getBoolean("validator.tst.enabled")) {
                it.add(
                    TimestampValidator(
                        config.getDuration("validator.tst.maxPast"),
                        config.getDuration("validator.tst.maxFuture")
                    )
                )
            }
            ...
            if (config.getBoolean("validator.oday.enabled")) {
                it.add(
                    OdayValidator(
                        ...
                        config.getInt("validator.oday.maxPast"),
                        config.getInt("validator.oday.maxFuture")
                    )
                )
            }
        }
    ...
    override fun handleMessage(msg: Message<Any>) {
        if (
            TransitdataSchema.hasProtobufSchema(msg, TransitdataProperties.ProtobufSchema.HfpData)
        ) {
            try {
