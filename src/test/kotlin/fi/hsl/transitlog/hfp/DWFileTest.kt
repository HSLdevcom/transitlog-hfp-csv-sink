...
class DWFileTest {
    private val testDataStartTime =
        ZonedDateTime.of(LocalDate.of(2021, 1, 1), LocalTime.of(8, 0), ZoneId.of("Europe/Helsinki"))

    private fun generateTestData(): List<Hfp.Data> =
        (1..10).map { i ->
            Hfp.Data.newBuilder()
                .setTopic(
                    Hfp.Topic.newBuilder()
                        ...
                        .setVehicleNumber(1)
                        .setReceivedAt(
                            testDataStartTime.plusSeconds(i.toLong()).toInstant().toEpochMilli()
                        )
                        .setRouteId("1")
                        .setStartTime("08:00")
                ...
                .setPayload(
                    Hfp.Payload.newBuilder()
                        ...
                        .setTst(
                            testDataStartTime.plusSeconds(i.toLong()).toOffsetDateTime().toString()
                        )
                        .setVeh(1)
                        .setSchemaVersion(1)
                )
    ...
    fun `Test creating blob identifiers`() {
        ...
        val event =
            Event(
                ...
                oday = ZonedDateTime.now(tz).toLocalDate()
            )
        ...
        val id3 =
            fileFactory.createBlobIdentifier(
                Event(
                    UUID.randomUUID(),
                    OffsetDateTime.now(),
                    eventType = "VP",
